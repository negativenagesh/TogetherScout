...
export default function Founders() {
    ...
    founders.forEach(founder => {
      setBios(prev => {
        ...
        if (founder.company_slug) {
          fetch('http://localhost:8000/api/companies/founders/bio', {
            method: 'POST',
  ...
  const fetchFounders = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/companies/founders_yc_search', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleAutopsy = async (founder) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/companies/founders/${founder.id}/evaluate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
